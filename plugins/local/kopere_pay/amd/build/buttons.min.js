define(["jquery"],function($){let buttons={init:function(){$(".link-area").click(function(event){event.preventDefault();let img=$(this).find("img").attr("src");$(".show-area").show(400);$(".show-area .img-area-p").attr("src",img);$(".show-area .img-area-m").attr("src",img);$(".show-area .img-area-g").attr("src",img);let url=$(this).attr("href").replace("#","");let texto_p='<a href="'+url+'">'+"\r\n"+'    <img style="height:30px" src="'+img+'"  alt="img">'+"\r\n"+"</a>";$("#copy-code-1").text(texto_p);$("#copy-code-12").text(texto_p.replace("/?","/view.php?"));let texto_m='<a href="'+url+'">'+"\r\n"+'    <img style="height:50px" src="'+img+'" alt="img">'+"\r\n"+"</a>";$("#copy-code-2").text(texto_m);$("#copy-code-22").text(texto_m.replace("/?","/view.php?"));let texto_g='<a href="'+url+'">'+"\r\n"+'    <img  style="height:70px" src="'+img+'" alt="img">'+"\r\n"+"</a>";$("#copy-code-3").text(texto_g);$("#copy-code-32").text(texto_g.replace("/?","/view.php?"))});$(".copy-code").each(buttons.copy_code_each)},copy_code_each:function(id,element){let $element=$(element);let copyElementId=$element.attr("id");function elementClick(event){let cell=document.getElementById(copyElementId);let range,selection;if(document.body.createTextRange){range=document.body.createTextRange();range.moveToElementText(cell);range.select()}else if(window.getSelection){selection=window.getSelection();range=document.createRange();range.selectNodeContents(cell);selection.removeAllRanges();selection.addRange(range)}}function elementCopy(event){if(document.body.createTextRange){navigator.clipboard.writeText($element.val()||$element.text())}else if(window.getSelection){navigator.clipboard.writeText($element.val()||$element.text())}$tolltip.find("span").html(M.util.get_string("copied","local_kopere_pay"))}let $tolltip=$(`<div class="copi-code-icon">
             ...
             </div>`);$element.click(elementClick).on("mouseout",function(){$tolltip.find(".text-copiar").html(M.util.get_string("copy","local_kopere_pay"))});$tolltip.click(elementCopy).on("mouseout",function(){$tolltip.find(".text-copiar").html(M.util.get_string("copy","local_kopere_pay"))});$element.parent().addClass("copy-code-area");$element.after($tolltip)}};return buttons});