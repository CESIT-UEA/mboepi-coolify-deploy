 ...
 * Class MeioAsaas
 */
class MeioAsaas implements IMeio {
    ...
    public static function get_name() {
        $habilitado = config::get_key("kopere_pay-habilitar-MeioAsaas");
        if ($habilitado) {
            set_config('form_pedircpf', 1, 'local_kopere_dashboard');
        }

        return [
            'name' => 'Asaas - Cartão de crédito',
    ...
    public static function is_enable() {
        if (config::get_key("kopere_pay-habilitar-MeioAsaas")) {
            return true;
        }
    ...
    public static function edit(form $form) {
        ...
        $form->add_input(
            input_text::new_instance()
                ...
                ->set_value_by_config('kopere_pay-meioasaas-installmentCount', 0)
                ->set_description("Defina a quantidade máxima de parcelas que o aluno poderá pagar!")
                ->set_type('number')
    ...
    public static function pay_button($detalhe, $course) {
        ...
        $input = "";
        $installmentCount = config::get_key('kopere_pay-meioasaas-installmentCount');
        ...
        $link = "{$CFG->wwwroot}/local/kopere_pay/?id={$detalhe->course}&meio=MeioAsaas&enroll=1";
    ...
    public static function enroll($course) {
        ...
        $enrollmentid = local_kopere_pay_enrollment::add_enrollment($USER->id, $id, 'MeioAsaas', $coupon, $price);
        ...
        if ($detalhe->charge == 'mensalidade') {
            ...
            $payments = (object) [
                ...
                "externalReference" => "M-{$enrollmentid}",
                "cycle" => "MONTHLY",
            ...
            $result = curl_exec($ch);
            if (curl_errno($ch)) {
                pay_header::notfound('<h2>Asaas respondeu com error:</h2>' . curl_errno($ch) . " =>" . curl_error($ch));
            }
            curl_close($ch);

            $return = json_decode($result);
            local_kopere_pay_history::add_history($enrollmentid, $payments, $return);

            header::location(
                "{$CFG->wwwroot}/local/kopere_pay/?id={$id}&completed=1&meio=MeioAsaas&enrollment={$enrollmentid}&vai=1"
            );
        } else {
    ...
    public static function returned() {
        ...
        $enrollmentid = intval(substr($returned->payment->externalReference, 2));
        if ($enrollmentid) {
            local_kopere_pay_history::add_history($enrollmentid, '', $returned);
    ...
    public static function completed($course, $user) {
        ...
        if ($detalhe->charge == 'mensalidade') {
            ...
            if (optional_param('vai', false, PARAM_INT)) {
                $sql =
                    "SELECT * FROM {local_kopere_pay_history} WHERE enrollmentid = {$enrollmentid} AND receive LIKE '%PAYMENT_CREATED%' LIMIT 1";
